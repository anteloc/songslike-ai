...
def process_song(
    ...
    Returns (mus_path, lrc_path) so the caller can report results without interleaved prints.
    """
...
def main() -> int:
    ...
    # Resolve lyrics paths before submitting so workers receive plain Path | None
    work_items: list[tuple[Path, Path | None]] = []
    for audio_path in audio_files:
        lrc = audio_path.with_suffix(".lrc")
        lyrics_path: Path | None = lrc if lrc.exists() else None
        label = "(no .lrc found — instrumental)" if lyrics_path is None else ""
        print(f"[{audio_path.name}] {label}".rstrip())
        work_items.append((audio_path, lyrics_path))

    errors: list[tuple[Path, Exception]] = []
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        futures = {
            executor.submit(process_song, audio_path, lyrics_path, output_dir, args.no_ts): audio_path
            for audio_path, lyrics_path in work_items
        }
        for future in as_completed(futures):
            audio_path = futures[future]
            try:
                mus_path, lrc_path = future.result()
                print(f"  {mus_path}\n  {lrc_path}")
